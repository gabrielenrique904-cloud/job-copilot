...
def puede_ejecutar_accion() -> bool:
    ...
    if ahora - ultima_accion < TIEMPO_MINIMO_ENTRE_ACCIONES:
        segundos_restantes = round(
            TIEMPO_MINIMO_ENTRE_ACCIONES - (ahora - ultima_accion)
        )
        st.warning(
            f"Espera {segundos_restantes} segundos antes de volver a intentarlo."
        )
        return False
...
with col1:
    archivo_cv = st.file_uploader(
        "O sube tu CV (PDF o Word)", type=["pdf", "docx"], key="upload_cv_analizar"
    )

    cv_texto_extraido = ""
    if archivo_cv is not None:
        cv_texto_extraido = extraer_texto_cv(archivo_cv)
        if cv_texto_extraido:
            st.success("CV cargado correctamente desde el archivo.")
        else:
            st.error(
                "No pudimos leer el archivo. Prueba con otro PDF/Word, o pega el texto manualmente."
            )

    cv_texto = st.text_area("Tu CV", value=cv_texto_extraido, height=300)
...
if st.session_state.resultado_analisis:
    ...
    if "error" in resultado:
        ...
        if st.button(
            "📄 Generar CV adaptado a esta oferta (PDF)",
            disabled=st.session_state.procesando_cv_manual,
        ):
...
st.write(
    "Pega tu CV y te buscamos ofertas reales publicadas en las últimas 24h, ordenadas por compatibilidad."
)

archivo_cv_busqueda = st.file_uploader(
    "O sube tu CV (PDF o Word)", type=["pdf", "docx"], key="upload_cv_busqueda"
)

cv_texto_extraido_busqueda = ""
if archivo_cv_busqueda is not None:
    cv_texto_extraido_busqueda = extraer_texto_cv(archivo_cv_busqueda)
    if cv_texto_extraido_busqueda:
        st.success("CV cargado correctamente desde el archivo.")
    else:
        st.error(
            "No pudimos leer el archivo. Prueba con otro PDF/Word, o pega el texto manualmente."
...
cv_busqueda = st.text_area(
    "Tu CV (para la búsqueda automática)",
    value=cv_texto_extraido_busqueda,
    height=200,
    key="cv_busqueda",
)
palabras_clave = st.text_input(
    "Palabras clave", value="customer success", key="palabras_clave"
)
...
ubicacion_busqueda = st.selectbox(
    "Ubicación", provincias_espana, index=0, key="ubicacion_busqueda"
)
...
if st.session_state.procesando_busqueda:
    ubicacion_para_busqueda = (
        None if ubicacion_busqueda == "Toda España" else ubicacion_busqueda
    )

    with st.spinner(
        "Buscando y analizando ofertas reales... esto puede tardar unos segundos"
    ):
        resultados = buscar_y_analizar_ofertas(
            cv_busqueda, palabras_clave, ubicacion=ubicacion_para_busqueda
        )
...
if st.session_state.ofertas_encontradas is not None:
    ...
    if len(ofertas) == 0:
        st.info(
            "No se encontraron ofertas para esos criterios en las últimas 24h. Prueba otras palabras clave."
        )
    else:
        st.success(
            f"Se encontraron {len(ofertas)} ofertas, ordenadas de mayor a menor compatibilidad:"
        )
        ...
        for oferta in ofertas:
            with st.container(border=True):
                ...
                with col_cv:
                    if st.button(
                        "📄 Generar CV adaptado", key=f"generar_cv_{oferta['url']}"
                    ):
                        with st.spinner("Generando tu CV adaptado a esta oferta..."):
                            cv_adaptado_texto = generar_cv_adaptado(
                                cv_busqueda, oferta["descripcion"]
                            )
                            ruta_pdf = crear_pdf_desde_texto(
                                cv_adaptado_texto,
                                ruta_salida=f"cv_{oferta['empresa']}.pdf",
                            )
